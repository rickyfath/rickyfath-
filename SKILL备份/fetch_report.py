...
def lookup_csv(code: str):
    ...
    if os.path.exists(CONCEPT_CSV):
        try:
            with open(CONCEPT_CSV, "r", encoding="utf-8-sig") as f:
                reader = csv.DictReader(f)
                for row in reader:
                    csv_code = row.get("code", "").strip()
                    # 去掉后缀: 002156.SZ → 002156
            ...
            if result["concept"] is None:
                result["concept"] = {"error": "not found in CSV"}
        except Exception as e:
            result["concept"] = {"error": f"CSV read error: {str(e)[:100]}"}
    else:
        result["concept"] = {"error": f"file not found: {CONCEPT_CSV}"}
...
def generate_report(raw_code: str):
        ...
        try:
            ...
            for cn, en in col_map.items():
                if cn in pl_df.columns:
                    val = latest[cn]
                    try:
                        ths_metrics[en] = float(str(val).replace('亿','').replace('万','').replace('%',''))
    ...
    report["g1_integrity"] = {
        "flags": integrity_flags if integrity_flags else ["无明显硬伤（基于CSV静态数据）"],
        "note": "完整 G1 需拉取 NonRec%/审计意见/商誉/大股东质押率 — 这些数据需 akshare 或手动查看财报"
    }
